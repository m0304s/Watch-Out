...
public class InferenceStartServiceImpl implements InferenceStartService {
    ...
    @Override
    public StartReport startAll(boolean mirror, boolean push) {
        List<StartReport.Item> items = new ArrayList<>();
        for (Cctv cctv : cctvRepository.findByType(Type.CCTV)) {
            items.add(callDetectStart(cctv, mirror, push));
        }
        return new StartReport(items);
    }

    @Override
    public StartReport startArea(UUID areaUuid, boolean mirror, boolean push) {
        List<StartReport.Item> items = new ArrayList<>();
    ...
    private StartReport.Item callDetectStart(Cctv cctv, boolean mirror, boolean push) {
        ...
        try {
            HttpResponse<String> response = client().postDetectStart(company, camera, src, mirror,
                push);
            ...
            return new StartReport.Item(cctv.getUuid(), camera, msg);
        } catch (Exception e) {
            log.error("[FastAPI] /detect/start failed for {} ({})", camera, src, e);
            return new StartReport.Item(cctv.getUuid(), camera,
                "fastapi exception: " + e.getClass().getSimpleName());
        }
    }
}
