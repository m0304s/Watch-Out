...
public class FcmTokenService {
     ...
     * FCM 토큰 등록
     */
    public FcmToken registerToken(FcmTokenRequest request) {
        UUID userId = SecurityUtil.getCurrentUserUuid()
            .orElseThrow(() -> new BusinessException(ErrorCode.INVALID_TOKEN));

        // 기존 토큰이 있는지 확인
        FcmToken existingToken = fcmTokenRepository.findByFcmToken(request.token());

        if (existingToken != null) {
            // 기존 토큰이 다른 사용자 것인 경우 업데이트
            if (!existingToken.getUserUuid().equals(userId)) {
    ...
    public FcmTokenResponse getMyTokens() {
        UUID userId = SecurityUtil.getCurrentUserUuid()
            .orElseThrow(() -> new BusinessException(ErrorCode.INVALID_TOKEN));
        List<FcmToken> tokens = fcmTokenRepository.findByUserUuid(userId);
        return FcmTokenResponse.from(tokens);
    }
}
