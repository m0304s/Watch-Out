...
export const login = async (
  ...
  // 로그인 성공 시 토큰을 메모리 스토어에 저장 (컴포넌트에서 setAuthData 호출이 기본 경로지만 안전망)
...
export const logout = async (): Promise<ApiResponse<void>> => {
  try {
    ...
    // 로그아웃 성공 시 메모리 스토어 초기화
    useAuthStore.getState().clearAuth()

    return response.data
  } catch (error) {
    // 에러가 발생해도 메모리 스토어 초기화
    useAuthStore.getState().clearAuth()
    throw error
  }
}
...
// isAuthenticated / getStoredToken: 메모리 스토어 기반으로 대체가 필요하면 훅에서 직접 사용
